...
export default function TradeModal({ event, tokenID, outcomeName, initialPrice, onClose }) {
  const { isConnected, onPolygon, switchToPolygon } = useWallet();
  ...
  const canTrade  = isConnected && onPolygon && sizeNum >= 1 && priceNum > 0;
      ...
      <div style={sheet}>
        ...
        {!isConnected ? (
          ...
          <div style={{ textAlign:'center', padding:'1.25rem 0' }}>
            ...
            <button style={primaryBtn} onClick={switchToPolygon}>Switch to Polygon</button>
          </div>
        ) : (
          <>
            {/* Outcome badge */}
            ...
            <label style={labelStyle}>
              ...
              <div style={inputWrap}>
                <input type="number" min="1" step="1" placeholder="e.g. 10"
                  value={size} onChange={e => setSize(e.target.value)} style={inputStyle} />
                <span style={inputSuffix}>USDC</span>
              </div>
            </label>
            ...
            {sizeNum > 0 && (
              <div style={summary}>
                ...
                <div style={summaryRow}>
                  ...
                  <span style={{ fontWeight:700, color:'var(--green)' }}>${sizeNum.toFixed(2)}</span>
                </div>
                <div style={summaryRow}>
            ...
            <div style={{ fontSize:'0.64rem', color:'var(--muted)', marginTop:'0.75rem', lineHeight:1.5, textAlign:'center' }}>
              Opens polymarket.com to complete your trade. Restricted in some jurisdictions including the US.
            </div>
          </>
        )}
      </div>
...
const closeBtn   = { width:28, height:28, borderRadius:'50%', background:'var(--bg)', border:'none', cursor:'pointer', fontSize:'0.85rem', color:'var(--muted)', fontFamily:'inherit' };
...
const inputStyle = { flex:1, padding:'0.65rem 0.75rem', border:'1.5px solid var(--border)', borderRadius:'10px', fontSize:'1rem', fontWeight:700, color:'var(--text)', outline:'none', fontFamily:'inherit' };
